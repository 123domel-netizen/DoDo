...
export function ItemTagsEditor({
  ...
  const assignedIds = effectiveTagIds(item, myTagIdsByItem);
  const assigned = resolveItemTags(assignedIds, tags);
  ...
  const toggle = (tagId: string) => {
    if (assignedIds.includes(tagId)) {
      onChange(assignedIds.filter((id) => id !== tagId));
    } else {
      onChange([...assignedIds, tagId]);
    }
  };
