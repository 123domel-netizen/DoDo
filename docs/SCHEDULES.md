...
| Dane | Warunek |
|------|---------|
| Supabase (wspólne dla zespołu) | zalogowany użytkownik + aktywna org + migracje `0054` + `0057` + `0058` + `0060` + `0062` + `0063` |
...
- Cloud: `SupabaseScheduleRepository` + migracje `0054_schedules.sql`, `0057_crew_attendance.sql`, `0058_crew_attendance_workers.sql`, `0062_crew_members.sql`, `0063_crew_viewer_user_ids.sql`
...
**Widoczność brygady:** `viewer_user_ids` (`0063`) — pusta lista = cały zespół; niepusta = tylko wskazani widzą brygadę i jej obecności (UI w edycji brygady + filtr list + RLS).
